...
const SpeechToText = () => {
  ...
  const analyzeMedicalConversation = async (conversation) => {
    ...
    try {
      ...
      // Handle nested JSON strings if present
      if (typeof data === 'string') {
        data = JSON.parse(data);
      }
      
      // Ensure all expected fields are arrays or proper types
      if (data.diseases && typeof data.diseases === 'string') {
        try {
          data.diseases = JSON.parse(data.diseases);
        } catch (e) {
  ...
  const verifyPrescription = async () => {
    ...
    try {
      ...
      if (data.red_flags && typeof data.red_flags === 'string') {
        try {
          data.red_flags = JSON.parse(data.red_flags);
        } catch (e) {
          data.red_flags = [data.red_flags];
        }
      }
      
      if (data.drug_interactions && typeof data.drug_interactions === 'string') {
        try {
          data.drug_interactions = JSON.parse(data.drug_interactions);
        } catch (e) {
          data.drug_interactions = [data.drug_interactions];
        }
      }
      
      if (data.medicine_reviews && typeof data.medicine_reviews === 'string') {
        try {
          data.medicine_reviews = JSON.parse(data.medicine_reviews);
        } catch (e) {
          data.medicine_reviews = [data.medicine_reviews];
        }
      }
                        ...
                        <div className="analysis-section">
                          ...
                          <ol className="ps-3">
                            {medicalAnalysis.diseases.map((disease, idx) => (
                              <li key={idx} className="mb-1">{disease}</li>
                            ))}
                          </ol>
                        </div>
                      )}
                ...
                {verificationResult && (
                  <>
                    ...
                    <div className="verification-result card">
                      ...
                      <div className="card-body">
                        <div 
                          className={`alert ${verificationResult.can_prescribe ? 'alert-success' : 'alert-warning'}`}
                          style={
                            !verificationResult.can_prescribe 
                              ? { backgroundColor: '#fff3cd', borderColor: '#ffc107', color: '#856404' }
                              : {}
                          }
                        >
                          <h6>
                            {verificationResult.can_prescribe ? '✅ APPROVED' : '⚠️ REQUIRES CAUTION (Risky)'}
                          </h6>
                          <p className="mb-0">
                            <strong>Safety Level:</strong> 
                            <span style={{ 
                              color: verificationResult.overall_safety?.toLowerCase().includes('safe') 
                                ? '#155724' 
                                : verificationResult.overall_safety?.toLowerCase().includes('risk') || verificationResult.overall_safety?.toLowerCase().includes('danger')
                                ? '#dc3545'
                                : '#856404'
                            }}>
                              {' '}{verificationResult.overall_safety?.toUpperCase()}
                            </span>
                          </p>
                        </div>
                        ...
                        {verificationResult.medicine_reviews && verificationResult.medicine_reviews.length > 0 && (
                          <div className="mb-3">
                            ...
                            {verificationResult.medicine_reviews.map((med, idx) => {
                              const isRisky = med.status === 'caution' || med.status === 'rejected' || !med.age_appropriate;
                              const isRedFlag = med.status === 'rejected';
                              
                              return (
                                <div 
                                  key={idx} 
                                  className={`medicine-review card mb-2 ${
                                    isRedFlag ? 'border-danger' : isRisky ? 'border-warning' : ''
                                  }`}
                                  style={{
                                    borderLeftWidth: isRedFlag || isRisky ? '4px' : '1px',
                                    borderLeftColor: isRedFlag ? '#dc3545' : isRisky ? '#ffc107' : undefined
                                  }}
                                >
                                  <div className="card-body">
                                    <div className="d-flex justify-content-between align-items-start">
                                      <div>
                                        ...
                                        <p className="mb-1">
                                          ...
                                          <span style={{ color: !med.age_appropriate ? '#ffc107' : '#28a745' }}>
                                            {med.age_appropriate ? '✅ Yes' : '⚠️ No (Risky)'}
                                          </span>
                                        </p>
                                        <p className="mb-0">
                                          <strong>Reason:</strong> 
                                          <span style={{ color: isRedFlag ? '#dc3545' : isRisky ? '#856404' : 'inherit' }}>
                                            {med.reason}
                                          </span>
                                        </p>
                                      </div>
                                    ...
                                    {med.contraindications && med.contraindications.length > 0 && (
                                      <div className="mt-2 alert alert-warning py-2 mb-0">
                                        <small>
                                          ...
                                          <span style={{ color: '#856404' }}> {med.contraindications.join(', ')}</span>
                                        </small>
                                      </div>
                                    )}
                                    ...
                                    {med.alternatives_if_rejected && med.alternatives_if_rejected.length > 0 && (
                                      <div className="mt-2">
                                        <small>
                                          <strong>💡 Suggested Alternatives:</strong> {med.alternatives_if_rejected.join(', ')}
                                        </small>
                                      </div>
                                    )}
                                  </div>
                        ...
                        {verificationResult.drug_interactions && verificationResult.drug_interactions.length > 0 && (
                          <div className="mb-3 alert alert-warning" style={{ backgroundColor: '#fff3cd', borderColor: '#ffc107' }}>
                            <h6 style={{ color: '#856404' }}>⚠️ Drug Interactions (Risky):</h6>
                            {verificationResult.drug_interactions.map((interaction, idx) => (
                              <div key={idx} className="mb-2" style={{ color: '#856404' }}>
                        ...
                        {verificationResult.red_flags && verificationResult.red_flags.length > 0 && (
                          <div className="mb-3 alert alert-danger" style={{ backgroundColor: '#f8d7da', borderColor: '#dc3545' }}>
                            <h6 style={{ color: '#721c24' }}>🚨 Red Flags (Critical):</h6>
                            <ul className="mb-0" style={{ color: '#721c24' }}>
                              {verificationResult.red_flags.map((flag, idx) => (
                                <li key={idx} className="mb-1">{flag}</li>
                              ))}
                            </ul>
                          </div>
                        )}
